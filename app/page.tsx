    ...
    <div className="min-h-screen bg-rgb-bg-primary theme-transition">
      <main className="p-4 sm:p-6 w-full space-y-6">
        <div className="max-w-7xl mx-auto">
          {/* Header Section */}
          <div className="mb-8">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-rgb-fg-primary">
            ...
            {stats.map((stat, index) => (
              <div key={index} className="stat-card">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-2xl font-bold text-rgb-fg-primary">
                      {stat.value}
                    </p>
                    <p className="text-sm text-rgb-fg-secondary">
                      {stat.label}
                    </p>
          ...
          {/* Main Dashboard Grid - Responsive Bento Box Layout */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {accessibleActions.map((action) => (
                    <Link
                      key={action.id}
                      href={action.href}
                      className="action-card-light group"
                    >
              ...
              <div className="space-y-3">
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="activity-item">
                    <div
                      className={`activity-icon ${activity.color} text-white`}
                    >
                      <i className={`${activity.icon} text-sm`}></i>
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium text-rgb-fg-primary">
                        {activity.title}
                      </p>
                    ...
                    <div className="text-xs text-rgb-fg-quaternary">
                      {activity.time}
                    </div>
                  </div>
                ))}
